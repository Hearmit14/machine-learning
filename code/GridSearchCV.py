...
spark = SparkSession.builder.config(
    "spark.debug.maxToStringFields", "100").getOrCreate()
conf = SparkConf().set("spark.driver.memory", "4g").set(
    "spark.driver.maxResultSize", "4g")
sc = spark.sparkContext(conf)


hive_context = HiveContext(sc)
hiveDataFrame = hive_context.sql(
    'select * from sdk_user.hejy_temp_model_22002015')
predictDataFrame = hive_context.sql(
    'select * from sdk_user.hejy_temp_model_all')
...
lr = LogisticRegression().setMaxIter(
    1000).setRegParam(0.1).setElasticNetParam(0.01)
...
hive_context.sql(
    'insert into sdk_user.hejy_temp_3 select id,rawPrediction,probability,prediction from tempTable limit 100')
...
grid = GridSearchCV(estimator=gridlr, param_grid=parameters,
...
cv = CrossValidator().setEstimator(lr).setEvaluator(
    BinaryClassificationEvaluator()).setEstimatorParamMaps(paramGrid).setNumFolds(3)
