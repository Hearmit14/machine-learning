...
textFile = sc.textFile(
    "/user/hive/warehouse/dw_stage_open/dw_stage_open.db/hejy_push/zhimaq_4_20190103_08_37_28.csv")
textFile.first()
...
# 单词个数
lines.map(lambda line: len(line.split(" "))).reduce(
...
# 方法2
pairRDD4.mapValues(lambda x: (x, 1)).reduceByKey(
...
df = spark.read.json(
    "file:#/app/hadoop/spark/examples/src/main/resources/people.json")
df.show()
...
peopleDF = sc.parallelize([("Justin", 19), ("Michael", 29), ("Andy", 30)]).map(
    lambda line: line.split(',')).map(lambda x: Row(**f(x))).toDF()
peopleDF = sc.textFile("file:#/usr/local/spark/examples/src/main/resources/people.txt").map(
    lambda line: line.split(',')).map(lambda x: Row(**f(x))).toDF()
...
sentenceData = spark.createDataFrame([(0, "I heard about Spark and I love Spark"), (
    0, "I wish Java could use case classes"), (1, "Logistic regression models are neat")]).toDF("label", "sentence")
...
df = spark.createDataFrame([(0, "I heard about Spark and I love Spark".split(
    " ")), (0, "I wish Java love heard case Spark".split(" ")), (1, "Spark Java models heard case".split(" "))], ["id", "words"])
...
data = spark.sparkContext.textFile("file:#/home/dev/hejinyang/iris.txt").map(
...
featureIndexer = VectorIndexer().setInputCol(
    "features").setOutputCol("indexedFeatures").fit(df)
...
lr = LogisticRegression().setLabelCol("indexedLabel").setFeaturesCol(
    "indexedFeatures").setMaxIter(10).setRegParam(0.3).setElasticNetParam(0.8)
...
bestThreshold = fMeasure.where(
    fMeasure["F-Measure"] == maxFMeasure).select("threshold").head()[0]
0.5487261156903904
